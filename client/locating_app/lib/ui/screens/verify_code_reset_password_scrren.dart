    ...
    return BlocConsumer<ForgotPasswordBloc, BaseState>(
      listener: (context, state) {
        if (state is LoadedState<String>) {
          if (state.data == 'verify_otp_success') {
            Navigator.pushNamed(context, Routes.createNewPasswordScreen);
          } else if (state.data == 'resend_success') {
            scaffoldKey.currentState.showSnackBar(
              new SnackBar(
                content: new Text(
                  Language.of(context).getText("register.resend_otp"),
                ),
              ),
            );
          }
                ...
                children: [
                          ...
                          new TextSpan(
                            recognizer: new TapGestureRecognizer()..onTap = () {
                              ...
                              BlocProvider.of<ForgotPasswordBloc>(context).add(
                                TypeEmailEvent(email, isResend: true));
                            },
                  ...
                  Container(
                    ...
                    child: ItemButton(
                      ...
                      onPress: () {
                        BlocProvider
                          .of<ForgotPasswordBloc>(context)
                          .add(VerifyOtpCode(textEditingController.text, email));
                      },
                    ),
                  ),
                ],
